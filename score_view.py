...
with open(os.path.join(THIS_DIR, TEMPLATES_BASENAME, "entry_piece.html"), 'r') as f:
...
def _searialize_exit_object(exit_obj: dict) -> str:
    ...
    return "{exit_type} {exit_repr}".format(
        exit_type = exit_obj["type"], exit_repr = exit_obj["repr"])

def _result_cell_ternary(
    result: dict, on_success: str, on_error: str, on_error_but_flaky: str) -> str:
    if result["ok"]:
        return on_success
...
def _timeline_svg_html(
    ...
    finish_x = finish_x if (finish_x - start_x >= MIN_PATH_LEN) else (start_x + MIN_PATH_LEN)
    x_coords.append(finish_x)
    x_coords.append(length)
    return ("<svg xmlns='http://www.w3.org/2000/svg' width='{length}' height='20'>"
...
def _populate_entries_view(
    *, sorted_test_results: List[dict], timer_program: str,
    ...
    result_iter = itertools.groupby(sorted_test_results, lambda e : e["comb_id"] + e["desc"])
    for k, g in result_iter:
        results_for_one_test = sorted(list(g), key = lambda e : e["repeat"]["count"])
        command_invocation = score_utils.make_command_invocation_str(
            timer_path = timer_program, params = results_for_one_test[0],
            indent = 2, working_directory = working_directory,
        )
        known_flaky_errors, flaky_errors = "false", results_for_one_test[0]["flaky_errors"]
        if len(flaky_errors):
        ...
        test_entry_html = ENTRY_PIECE_TEMPLATE.format(
            ...
            flaky_error_count = sum(1 for e in results_for_one_test if e["error_is_flaky"]),
...
def _populate_html_template(
    *, html_dir: Path, test_dir: Path, test_title: str,
...
def _generate_web_view_impl(
    *, test_title: str, sorted_test_results: List[dict], disabled_cases: List[dict],
    master_log_path: Path, timer_program: str,
    test_dir: Path, html_dir: Path, html_file_path: Path,
    ...
    shutil.copytree(
        src = os.path.join(THIS_DIR, "static"),
        dst = str(html_dir.joinpath("static"))
    )
    html_str = _populate_html_template(
        html_dir = html_dir, test_dir = test_dir, test_title = test_title,
...
def generate_web_view(
    *, test_title: str, test_master_log: str,
    disabled_cases: List[dict], timer_program: str, to_dir: str,
    working_directory: Optional[str] = None) -> str:
    ...
    _generate_web_view_impl(
        ...
        test_dir = test_dir, html_dir = html_dir,
...
def main():
    ...
    html_file_path = generate_web_view(
        test_title = args.title, test_master_log = args.log,
        disabled_cases = [], timer_program = args.timer,
        to_dir = args.to_dir,
    )
